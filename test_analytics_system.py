...
def test_reporting_service_with_stubbed_init():
    ...
    We only stub AnalyticsReportGenerator.__init__ since that's what gets instantiated.
    """
    
    # Only need to stub the main class that gets instantiated
    with patch.object(AnalyticsReportGenerator, '__init__', return_value=None):
        # Use parrot for the class method
...
def test_reporting_service_with_approval():
    ...
    # Only stub the main class being instantiated
    with patch.object(AnalyticsReportGenerator, '__init__', return_value=None):
        with parrot(AnalyticsReportGenerator.generate_summary):
            service = ReportingService()
